...
use super::docs::{forgotten_hashes, render_forgotten, render_store_index, subject_matches};
use super::layout::{blob_path, store_dir};
use super::model::{Forgotten, Subject};
use super::paths::slash_path;
use super::read::describe_unreadable;
use super::{EVENTS_DIR, FORGOTTEN_STEM, HistoryReadHost, HistoryStore, HistoryWriteHost};
...
impl<H: HistoryWriteHost> HistoryStore<H> {
    ...
    ///
    /// Blobs are deleted outside the change set for [`prune`](Self::prune)'s
    /// reason: a staged removal buffers the bytes it deletes in order to be able
    /// to put them back, which is the one thing a destruction verb must not do.
    ///
    ...
    pub async fn forget(
        ...
        // what to do about it.
        if !force && let Some(live) = self.subject_live(&root_doc, subject).await? {
        ...
        let (forgotten_path, present) = self.forgotten_path(&store_index).await?;
        ...
        let text = render_forgotten(
            ...
            subject,
            now,
            self.host().default_embed_format(),
        )?;
