...
fn a_captured_workspace_goes_back_from_its_blobs_without_a_journal_its_size() {
    // What restore rests on, proved against what Phase 0 actually writes: a
    // manifest plus `blob_path` is enough to stage the whole capture set as
    ...
    let mut cs = w.host_mut().change();
    for file in &event.files {
        cs.copy_from(&file.path, blob_path(store_index, &file.hash).unwrap());
    }
    let journal = prov_transaction::journal::encode(cs.ops()).unwrap();
    assert!(
        journal.len() < 2048,
    ...
    block_on(w.host_mut().commit(cs)).unwrap();
...
fn restore(
    ...
    let event = block_on(w.event(root, id))?.expect("the event should be in the store");
    let plan = block_on(w.restore_plan(root, &event, scope, exact))?;
    block_on(w.restore(root, &plan, force))?;
    Ok(plan)
}
...
fn a_default_restore_deletes_nothing_and_exact_makes_the_tree_match() {
    ...
    let mut w = store(&dir);
    let root = Path::new("index.md");
    let event = block_on(w.event(root, &id)).unwrap().unwrap();
    let additive = block_on(w.restore_plan(root, &event, &Scope::Whole, false)).unwrap();
    let exact = block_on(w.restore_plan(root, &event, &Scope::Whole, true)).unwrap();

    assert_eq!(additive.count(Disposition::Remove), 0);
    block_on(w.restore(root, &additive, false)).unwrap();
    assert!(
    ...
    block_on(w.restore(root, &exact, false)).unwrap();
...
fn a_restore_refuses_to_displace_a_registration_unless_it_resolves_it_itself() {
    ...
    let ev = block_on(w.event(Path::new("index.md"), &event))
        .unwrap()
        .unwrap();
    let plan = block_on(w.restore_plan(Path::new("index.md"), &ev, &Scope::Whole, false)).unwrap();
    ...
    let err = block_on(w.restore(Path::new("index.md"), &plan, false)).unwrap_err();
    ...
    // match this capture".
    let exact = block_on(w.restore_plan(Path::new("index.md"), &ev, &Scope::Whole, true)).unwrap();
    assert!(
    ...
    block_on(w.restore(Path::new("index.md"), &exact, false)).unwrap();
...
fn a_scope_restores_a_slice_and_refuses_what_the_capture_never_held() {
    ...
    let ev = block_on(w.event(Path::new("index.md"), &event))
        .unwrap()
        .unwrap();
    let plan = block_on(w.restore_plan(
        Path::new("index.md"),
        &ev,
    ...
    let by_id = block_on(w.restore_plan(Path::new("index.md"), &ev, &Scope::Id(id.clone()), false))
        .unwrap();
    assert_eq!(
        by_id
            .ops
    ...
    block_on(w.restore(Path::new("index.md"), &by_id, false)).unwrap();
    ...
    for scope in [
        ...
        assert!(block_on(w.restore_plan(Path::new("index.md"), &ev, &scope, false)).is_err());
    }

    // And `exact` is a statement about the whole tree, which a slice of the
    // capture cannot make.
    assert!(
        block_on(w.restore_plan(
            Path::new("index.md"),
            &ev,
            &Scope::Paths(vec![PathBuf::from("notes")]),
            true,
...
fn a_restored_root_never_strands_the_store_unreachable() {
    ...
    // invisible to the store's own checks, and unfindable by the next restore.
    let dir = seed("restore-pointer");
    let Captured::Written { id, .. } = capture(&dir, "2026-07-31T09:15:22Z", None) else {
        panic!("the first capture must write an event");
    };
    let rootless = "---\ntitle: Home\ncontents:\n- notes/a.md\n- notes/photo.jpg.yaml\n---\nroot\n";
    let hash = digest(rootless.as_bytes());
    ...
    let mut w = store(&dir);
    let mut event = block_on(w.event(Path::new("index.md"), &id))
        .unwrap()
        .unwrap();
    ...
    let plan =
        block_on(w.restore_plan(Path::new("index.md"), &event, &Scope::Whole, false)).unwrap();
    block_on(w.restore(Path::new("index.md"), &plan, false)).unwrap();
    ...
    assert!(
        block_on(HistoryReadHost::history_path(
            w.host(),
            Path::new("index.md")
        ))
        .unwrap()
...
/// `cfg(target_os)`, mirroring the production probe this exercises. Every
/// test below that depends on case-folding actually happening skips its
/// case-insensitive-only assertions when this is `false`, so the suite stays
/// green on Linux CI.
...
/// on-disk-identity probe reads, so a test can assert *which* casing
/// survived rather than merely that *a* casing did.
...
fn a_foreign_event_naming_two_paths_that_differ_only_by_case_is_refused_exactly_where_the_filesystem_would_self_clobber()
 {
    ...
    let result = block_on(w.restore_plan(Path::new("index.md"), &event, &Scope::Whole, false));
    if case_insensitive_fs(&dir) {
        assert!(
            result.is_err(),
