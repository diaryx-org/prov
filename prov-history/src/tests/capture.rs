...
fn the_event_document_matches_the_format_spec_byte_for_byte() {
    ...
    let root_hash = digest(read(&dir, "index.md").as_bytes());
    let note_hash = digest(read(&dir, "notes/a.md").as_bytes());
...
fn the_store_is_authored_in_the_workspaces_own_grammar_not_just_its_extension() {
    ...
    assert_eq!(
        block_on(html.list(Path::new("index.html"))).unwrap().len(),
        1
    );
}
...
fn a_capture_bootstraps_the_store_and_captures_attachment_payloads() {
    ...
    let blob = blob_path(Path::new("history/index.md"), &digest(b"JPEGBYTES")).unwrap();
...
fn capture_sorts_the_manifest_byte_wise_not_by_path_components() {
    ...
    let recomputed = digest(&canonical_bytes(
        &event.created,
        &event.trigger,
    ...
    assert_eq!(
        &id[id.len() - 8..],
        &recomputed["sha256:".len().."sha256:".len() + 8]
    );
}
...
fn the_store_is_never_captured_into_itself() {
    ...
    let set = block_on(store(&dir).capture_set(Path::new("index.md"))).unwrap();
    assert!(
...
fn the_first_event_records_the_root_that_already_declares_the_store() {
    ...
    let on_disk = digest(read(&dir, "index.md").as_bytes());
    assert_eq!(
...
fn concurrent_captures_on_two_devices_merge_without_conflict() {
    ...
    for body in ["from device one", "from device two"] {
        let hash =
            digest(format!("---\ntitle: A\npart_of: '../index.md'\n---\n{body}\n").as_bytes());
...
fn a_merged_shard_index_is_reported_stale_and_rebuilt_from_its_directory() {
    ...
    // (`prov` runs the other half: that this issue surfaces from `check` and
    // that its autofix retires it.)
    ...
    // The store names it, and names exactly the one shard that drifted.
    let issues = findings(&one);
    let [HistoryIssue::IndexStale { index, .. }] = issues.as_slice() else {
        panic!("expected one stale shard: {issues:?}");
    };
    assert_eq!(index, &PathBuf::from("history/events/2026/07/index.md"));

    // …and the repair is a pure function of that directory's listing, so
    // writing it back settles the finding.
    let rebuilt = block_on(store(&one).index_text(index)).unwrap();
    write(&one, index.to_str().unwrap(), &rebuilt);
    assert_eq!(findings(&one), vec![], "the rebuild should have settled it");
...
fn a_capture_after_a_merge_records_the_merged_state() {
    ...
    let events = block_on(store(&one).list(Path::new("index.md"))).unwrap();
...
// ---- the fixity cache ----
//
// The host supplies the cache ([`HistoryWriteHost::fixity_cached`] /
// [`HistoryWriteHost::fixity_remember`]) and `prov_fixity` supplies the
// validator behind it. What these pin is the half in between, which is
// capture's: that a warm cache actually removes the reads, that it cannot hide
...
fn a_warm_cache_captures_without_reading_a_single_file() {
    ...
    let cache = warm.host_mut().take_fixity_cache().unwrap();
