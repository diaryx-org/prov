...
pub(crate) fn cmd_convert(
    ...
    match axis {
        "path_style" | "path-style" => {
            ...
            let style = LinkStyle::from_axes(session.ctx.config.notation, ps);
            let changed = block_on(session.ws.convert_link_style(
                &ws_rel(&session.ctx, file)?,
                style,
                recursive,
            ))?;
            session.commit()?;
        ...
        "notation" => {
            ...
            let style = LinkStyle::from_axes(nt, session.ctx.config.path_style);
            let changed = block_on(session.ws.convert_link_style(
                &ws_rel(&session.ctx, file)?,
                style,
                recursive,
        ...
        "metadata.format" | "metadata_format" | "format" => {
            ...
            let changed = block_on(session.ws.convert_meta_format(
                &ws_rel(&session.ctx, file)?,
                fmt,
                recursive,
            ))?;
        ...
        "metadata.embed" | "metadata_embed" | "embed" => {
            ...
            let changed = block_on(session.ws.convert_meta_embed(
                &ws_rel(&session.ctx, file)?,
                style,
                recursive,
            ))?;
        ...
        "content_format" | "content-format" | "content" => {
            ...
            let changed = block_on(session.ws.convert_content_format(
                &ws_rel(&session.ctx, file)?,
                fmt,
                recursive,
                force,
