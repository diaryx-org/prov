...
fn the_store_is_authored_in_the_workspaces_own_grammar_not_just_its_extension() {
    ...
    let Captured::Written { id, .. } =
        block_on(w.history_capture(Path::new("index.html"), "2026-07-31T09:15:22.000000Z", None))
            .unwrap()
    else {
        panic!("the first capture must write an event");
    };
...
fn a_warm_cache_captures_without_reading_a_single_file() {
    ...
    fs.reset();
    let third =
        block_on(ws.history_capture(Path::new("index.md"), "2026-01-03T00:00:00Z", None)).unwrap();
    assert_eq!(
        fs.total_byte_reads(),
        0,
...
fn a_cached_manifest_is_the_manifest_the_disk_would_have_given() {
    ...
    block_on(warm_ws.history_capture(Path::new("index.md"), "2026-01-02T00:00:00Z", None)).unwrap();
    ...
    block_on(ws(&cold).history_capture(Path::new("index.md"), "2026-01-03T00:00:00Z", None))
        .unwrap();
    block_on(warm_ws.history_capture(Path::new("index.md"), "2026-01-03T00:00:00Z", None)).unwrap();
...
fn an_edited_file_is_still_captured_with_a_warm_cache() {
    ...
    let events =
        block_on(crate::history::tests::support::ws(&dir).history_list(Path::new("index.md")))
            .unwrap();
    let row = events
        .last()
        .unwrap()
