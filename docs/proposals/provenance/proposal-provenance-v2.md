...
## Status: implemented (2026-09-11), unreleased

Built the day it was accepted, in the commit that closes this document: the
`provenance` module, `prov confirm`, and the `confirmation_stale` finding, with
[`docs/provenance.md`](/docs/provenance.md) as the guide. The release that
carries it is Adam's to name.

Two things moved between the draft and the build, and both are in the guide.
The verb is **`confirm`** and the field **`confirmed`**, not `vouch`: a review
of the historica family found that *vouch* already means "a trusted key signed
this chain" in historica's decision 0046, in historica-minisign, and in diaryx's
own vouch module and user-facing verdict, and one word for two strengths of
claim in one app is the confusion §5 exists to prevent. That review also drew
the line this family sits on: a confirmation is the **keeper's own ledger**, a
first-person line in the document that whoever can edit the document can
write, as trustworthy as `author` and no more — the official record of what
was checked. The signed, second-person vouch — a reviewer's own record, in
their own history, that the document's keeper cannot edit — is
historica-minisign's subject, and a remark in historica-remark is the shape a
cross-person review takes. Neither is attempted here; §8 says why the two
compose rather than compete.

And the finding fires only while **nothing stands**. The draft had it fire
whenever a stale entry existed, which — since entries are never dropped —
would have fired forever after a re-confirmation. A live entry is one made
after the last change, so the document has been confirmed since and its older
entries are history.

### The draft, as it stood before the build
...
- **A confirmation is bound to the `updated` stamp, not to a fixity digest.** A
  ...
  The question a confirmation has to survive is *did this change after someone
...
- **A bare actor is a human; a non-human must carry a prefix.** v1 asked for a
  ...
  once told.
- **The verb is `confirm` and the field is `confirmed`.** OKF's `verified` reads
  as though something was checked mechanically, which is the confusion §5 of
  v1 existed to prevent. *Confirm* says what the act is: a person staking their
  name on a document being right. It is intentional, and it is honest about
  being an assertion rather than proof — and it is not
  *vouch*, which the historica family already uses for the signed act.
...
Still open, and listed in §11: whether the `confirmed` list is sorted so two
...
`generated: {by, at}` records how a document came to exist. `confirmed: [{by,
at, of?}]` is an append-only list of dated, attributed confirmations that
someone read the document and found it correct. A confirmation is **stale** when the
document's `updated` stamp is newer than the confirmation's `at` — someone confirmed
this, and then it changed — and `check` says so with a finding. `prov confirm`
appends an entry; nothing else writes one.

The distinction: **`check` is the attester; `confirmed` is the record.** A confirmation
...
is. Neither substitutes for the other: prov never writes a confirmation from a
passing `check`, and a stored confirmation never suppresses a finding.
...
Nearer home, the historica family already covers the signed, per-revision
half of this question. historica-minisign is "who vouches for a revision": one
key, one revision digest, one role, one moment, minisign-signed, filed beside
the history, with the keys a copy believes kept device-local and never
travelling. diaryx signs every publish with it and derives a verdict of
vouched, unvouched, or refused — derived and never stored, the same posture as
§4. historica's own `author` and `blame` answer *who wrote this line* better
than a `generated.by` could, in a workspace historica records. What none of
them is, is a line in the document that a reader with `cat` and no store can
see: a claim covers a whole tree at one revision and lives beside the history,
not in the page. That is the gap this family fills, and §8 says how the two
compose.

v1 took the vocabulary and rejected the posture. This draft keeps that stance
and diverges further in spelling — `confirmed` for `verified`, bare-is-human for
...
Prose prov itself writes already says so. `about.md` is a pure function of
configuration, rewritten whole (spec §4, *generated prose*), and it carries a
`generated_by: prov <version>` byline for exactly this reason. It does not gain
a clock: a page that is a pure function of its inputs cannot carry the instant
it was written without ceasing to be one, and the byline without an `at` is
its `generated` in prov's own spelling.
...
| Tier | Condition |
| --- | --- |
| unconfirmed | no live `confirmed` entry |
| machine-confirmed | live entries, every actor prefixed |
| human-confirmed | at least one live entry with a bare actor |
...
**Where a person's name comes from.** `prov confirm` needs an actor, and who is
...
device-local default: `--by` > `PROV_ACTOR` > a one-line `actor` file beside the peer
map, the precedence the peer map already uses. It is not a workspace config key. A
confirmation with no actor from any of those is refused rather than written
anonymously, since an unattributed confirmation is the free-floating assertion this
...
that `fields` cannot yet say is a path into a list — `confirmed[].by` — which is
the same extension path-valued fields need for `sources[].resource`, and is
that proposal's to add.

## 5. Confirming vs. attesting — the load-bearing distinction
...
**prov never writes a confirmation from a passing `check`.** A green check attests
...
fraud the tiers exist to prevent. A confirmation is written when a person or a named
process asks for one, never on prov's initiative. Same shape as `prov id
--workspace`: minted on request, never unbidden.

**A stored confirmation never suppresses a finding.** You cannot confirm your way out
of a fixity mismatch. A confirmation is about meaning, a finding is about state, and
neither is evidence for the other. Concretely: no finding kind ever consults
`confirmed` when deciding whether to fire.

**A confirmation is scoped to the document as it stood.** This is the addition, and
...
A confirmation is **stale** when the document's `updated` instant is newer than the
confirmation's `at`. Both are the same clock in the same fixed-width format, so the
...
field is empty or absent has nothing saying it changed, and its confirmations
stand.
...
fails on its own terms, because appending the confirmation changes the file, so the
hash could only describe a file nobody can reproduce afterward.

Where a digest does exist, the confirmation names it too:
...
is the digest on record at the moment of confirming. A confirmation is then also stale
...
live with; a confirmation inherits it rather than adding to it. A workspace with the
...
**`prov confirm <doc>` never stamps `updated`.** Appending a confirmation is not an
edit to the content, and a verb that bumped the stamp it is compared against
would make every confirmation fresh by construction.

The finding:

> **`ConfirmationStale`** — the document's newest `confirmed` entry has an
> `at` older than the document's `updated`, or an `of` that does not match the
> current `content_hash`, and nothing has confirmed it since. Someone confirmed
> this, and then it changed.

Not an error: an edit after a review is the ordinary course of events. It is a
*demotion* — the document is unconfirmed again — and the finding is how a reader learns
that the assurance in the frontmatter describes a document that no longer
exists. The remedies are the two real ones: confirm again, or leave it and
...
stays; the tier calculation just stops counting it, and a document confirmed
again after an edit is not reported for the entries it outgrew.

## 6. The verb

```
prov confirm <doc> [--by <actor>]
```

Appends one entry to `confirmed`, with `at` from the CLI's clock and `by`
resolved as §4 says, and `of` when the document carries a digest. Refuses a
machinery store (§9) and a document it cannot parse. A second confirmation by the
same actor on an unchanged document is appended, not deduplicated — it is a
second fact, and the list is the record of them. A document whose recorded
checksum has drifted is refused: that is already a fixity mismatch, and a
confirmation cannot vouch its way past one.
...
that a confirmation is not one.

Why not `vouch`, which this draft first chose: across historica's decision
0046, historica-minisign, and diaryx's own vouch module, *vouch* already means
"a trusted key signed this chain", and a document tier of "unvouched" beside a
chain verdict of "unvouched" in one app would be one word for two strengths of
claim. *Confirm* is what a person does when they say they read something and
found it right, which is exactly the act, and it is the word this document
had been using for the noun all along.
...
(`sources[].resource`, `confirmed[].by`), and the rule that a declared
path-valued field is a target and not a string. Nothing else in this document
depends on `sources`.

## 8. Non-goals

- **Signatures and crypto.** `confirmed: [{by: amh}]` is an assertion, not
  proof, and this proposal does not make it one. The signed form exists and
  composes from outside: capture the workspace in a history, and a reviewer
  vouches for the revision in their own signed claim — historica-minisign's
  subject — with the confirmation line inside it. prov names no tool and
  learns nothing about chains; the pipe is the integration. Key management inside a
...
- **Attested Computation.** OKF's type — `runtime`, `parameters`, `executor`,
  ...
  confirmation applies to a view document exactly as to any other. That is the whole
  overlap.
...
manifest store are whole-file record stores prov re-lays-out, and a confirmation on
one would be a claim about a file prov rewrites. `prov confirm` refuses them.
`about.md` is generated prose, rewritten whole; its `generated_by` byline is
its `generated` in prov's own spelling, and it cannot be confirmed, since prov
would discard the entry on the next regeneration.
...
The field is `confirmed`, not `verified`; a bare actor is a person, not an
...
out — `confirmed` to `verified`, a bare actor to `human:`, `of` dropped.
...
1. **Concurrent appends to `confirmed`.** Two devices each append an entry and
...
2. **`stale_after`: absolute or relative?** An absolute date needs no
   arithmetic and rots; a relative span measured from the newest standing confirmation
...
3. **A config axis.** v1 proposed `provenance: on|off`. With `confirmed` written
   only on request and `generated` written only by tools that choose to, there
   may be nothing to switch: a workspace that never runs `prov confirm` has no
...
it wants it. Costs one docs page and zero code.

**Phase 1 — `generated`, `confirmed`, `prov confirm`, `ConfirmationStale`.** The core:
the actor grammar of §4, the staleness rule of §5, the verb of §6, and the
derived tier. Depends on nothing outside this document. `about.md` already
carries its byline, which is its `generated` in prov's own spelling.
