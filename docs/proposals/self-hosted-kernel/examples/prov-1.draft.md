...
<!--
  ...
  This is what `prov about` would generate for *this repository*, whose
  settings are close to the defaults. Compare it with ORGANIZATION.md, the
  same generator run against a workspace with its own vocabulary. The
  difference in length is the point: there is less to tell a stranger about
  a workspace that does the ordinary thing.
-->
...
written in YAML. Everything after it is the document. The order of the keys
is for your benefit, not the machine's.

Some files are metadata all the way down, with no document part —
`prov.yaml` is one. You can tell by the extension: `.yaml`, `.yml`,
`.json`, `.toml`, `.fig`, and `.figl` files have no separate body and no
fence.
...
target is in the parentheses. A target beginning with `/` is a path from
**this directory**, the top of the workspace; it is not a path from the
root of your filesystem. Anything else is a path relative to the file you
found it in. Fold `.` and `..` yourself, and do not follow symlinks.

Two other spellings mean the same thing and are understood wherever a
reference can appear, so you may meet them in files someone edited by
hand:

| written | called |
| --- | --- |
| `[[docs/spec.md]]` or `[[docs/spec.md\|Spec]]` | a wikilink |
| `docs/spec.md` | a bare target |

A target containing `://`, or beginning with `mailto:`, points outside this
directory and is never resolved. A target naming a file that is not here is
simply broken — worth noting, not a reason to stop reading.
...
| `links` | cross-references to other documents | many | `link_of` |
| `link_of` | documents that cross-reference this one | many | `links` |

Both halves of a pair are kept in step: if A lists B under `contents`, B
lists A under `part_of`. If you edit one half by hand and not the other,
...
with a single top. A document that belongs to several things says so with
`links` instead.

## Files that are not part of the tree

Following `contents` will never reach `prov.yaml`. That is deliberate, not
an omission.

`README.md` points at it directly, through a key named `config`, and
`prov.yaml` points at nothing in return. It holds this directory's
settings — the source this page was generated from. Files reached that way
are machinery: they are not documents in the tree, they carry no `part_of`,
and they are not counted when something asks what this workspace contains.

A binary file is never part of the tree either. To bring in an image or a
PDF, a small text file is created beside it that names it under a `content`
...
- **Identity.** A document earns a permanent id the first time something
  ...
  stamped into the document's own metadata, and a registry file mirrors it.
  Because the id lives in the file, it survives being moved or copied.
...
Two fields are worth leaving alone, both because something else may already
depend on them:
...
- **`content_hash`** — a checksum of an attachment's bytes. Changing it by
  hand asserts something about the payload that may not be true.

The relation fields are meant to be edited by hand. That is the whole point
of keeping them in the files.

---

<sub>Generated by prov 0.1.0 from this workspace's configuration. Edits to
this file will be overwritten — change `prov.yaml` instead, or run
