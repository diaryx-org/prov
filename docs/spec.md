...
| **Opaque payload** | the `content` field | *not a node* — the bytes are the body of a sidecar node (an attachment); hashed for fixity, never parsed. A body reference to the payload (`![…](photo.jpg)`) is not an edge, but it is **rewritten on move** as a reference to the sidecar: the payload travels with its node, and every reference to where it was is respelled for where it is |
