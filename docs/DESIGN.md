...
still refuses to descend into the directories `parked_dirs` names — a historica
store beside the root, and the recycle bin's `items/`. Those are unreached
because they are byte-parking stores (§4), not because nobody has got round to
linking them, and a sidecar minted beside a revision document would be adopting
another tool's store as content.
...
| History (a historica store, scoped by prov) | `history`, `config` | ✅ **Retired as prov's own store; rebuilt as a skiplist.** Recording plaintext is [historica](https://crates.io/crates/historica)'s whole subject (revisions, CRDT merge, amendment, forgetting, a store a person can read), and prov's captured-pre-image event store (`docs/history-format.md`, now superseded and kept for the stores already on disk) could only fall behind it. What prov still knows — the graph's bounded reachable walk, the bin's parked bytes, the derived page, manifest-claimed archives — lands in the one place historica reserves for it: `history/skipped.txt`. The `prov-history` crate computes a marker-fenced **generated region** of skip rules (regenerated whole, like a changelog's), scoping `historica record` to exactly the reachable graph; `prov history-skips` prints the plan and `--write` (gated on `history: manual`) rewrites the region. Two refusals keep the store sound: a rule is withheld rather than written when it would cover a path historica is tracking (a covering rule stops recording cold), and a hand-written rule shadowing a reachable file is reported, never repaired. A `history/` directory holding the historica marker is parked out of every workspace walk — not indexed, not orphan-swept, not validated (`historica check` is the authority on its interior); a folder merely *called* `history` is content like any other. The old `history` pointer relation survives only to keep an unmigrated event store parked. |
