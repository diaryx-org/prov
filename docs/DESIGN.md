...
| Single-document edits | `edit` | ✅ format-preserving `set`/`unset` over text; `prov edit` opens `$EDITOR` and, on a real content change, calls `Workspace::record_content_update` — one crash-safe write that restamps the fixity checksum (for a document that gets one — see the fixity row) and stamps the configured `updated` field (empty = off) with an RFC 3339 UTC instant the CLI supplies (the library stays clockless, DESIGN §2; the CLI's `now_rfc3339` is the workspace's one clock, microsecond-precise and fixed-width so a value is orderable as well as readable). Gated on actual change (byte-compared across the editor), so an open-and-quit stamps nothing |
