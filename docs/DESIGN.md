...
(`WorkspaceConfig::relation_set`). The diaryx preset is the **base** the
declarations overlay, not a fallback they displace: a declared name is added or
redefined, `<name>: off` retracts one, and a workspace that declares nothing gets
the preset unchanged. Wholesale replacement is declare-plus-`off`, spelled out,
because the alternative made extending the vocabulary by one pair cost a
restatement of the other four — and made a block that forgot to do so lose its
spine. The single-parent invariant is *checked*: `check` flags a spanning
...
One precomputed inverse exists now, and it is deliberately *not* in the store.
`retitle` and `rename` need every document that links to one, and the census
answered that by reading the whole reachable tree on every call. The
**inbound index** (`prov/src/workspace/inbound.rs`) is that census's inverse,
held in memory on the `Workspace` from one verb to the next — never persisted,
because a file every mutation rewrites is exactly the contention hotspot above,
and the CLI builds a fresh workspace per command anyway. It is derived, so it
heals toward the census and never the reverse: the public `backlinks` still
reads the census. What invalidates it is the two writers there are. prov's own
writes all land through `apply_set`, which re-derives each rewritten document's
edges from the staged bytes and drops the index whole when a set changes a
spanning entry, writes a document it does not know, moves or removes anything,
carries a registry write, or fails. Writes prov did not make — a consumer's own
filesystem handle, a sync — are found by a **stat sweep** on every ask: each
document's modification time and length as read, and each spanning target
that was missing confirmed still missing. A stat opens nothing, so on a
coordinated filesystem it is the one probe that costs no download; what it
cannot see is a same-length rewrite inside one modification-time tick, which
leaves one label or link for `check` to report. A backend with no modification
times gets no index and pays the census, as before.

The census the index is built from is the read that visits everything, and a
backend priced per *call* — a coordinated filesystem under a sync daemon,
where every read is a round trip before the first byte — pays for it once per
document unless it is told a whole-tree read is happening. The **bulk-read
hook** (`prov-graph/src/bulk.rs`, `BulkReads`, installed through the builder
or `Workspace::set_bulk_reads`) is that notice: the walk announces `begin`
under the graph's root before its first read and `end` after its last, and
*before it returns*, so a verb that walks and then writes has closed the
backend's scope by the time its change set lands, and a backend whose scope is
a read lock over the root is never asked to write inside it. The read memo
keeps what the walk read for the rest of the operation, so the rewrites a
mutation loads afterwards cost the backend nothing. A graph built without a
hook is told nothing and behaves as before.
...
deletion log, and flat vocabulary stores). Because it is a
...
The same rule governs the deletion log and *flat* vocabulary stores; a
...
reported as links or images. Not ordinary prose: the objection is that `[[…]]` may
be code (`[[None] * width]`) and a lexical scan cannot tell, which is why a
wikilink — whose span is lexical even with twig masking code — stays
diagnosis-only while an inline `[label](target)` does not. An image may be
retargeted but never unlinked: a link's label is prose that stays, an image's alt
is not. And never bytes: a repair may drop a link, but destroying a file is what a
verb asked for by name (`rm`, `empty-bin`) is for.
...
still refuses to descend into the directories `parked_dirs` names — a retired
recycle bin's `items/`, and a retired event store's archive. Those are unreached
...
| Mutation with link maintenance | `mutate` | ✅ `create`/`rename`/`move_tree`/`delete`/`restore`/`clear_deletions`/`adopt`/`separate`/`combine`/`duplicate` (parent entry, inverse links, re-relativization, labels kept; fig `Embed` edits). `move_tree` moves a **directory** as one change set — every document under it a mover, its inbound half one census folded over every path that moves, each mover's own links respelled with its fellow movers' new places already known, files prov does not read carried as bytes, the emptied directory removed afterwards — where a consumer moving a book as N `rename`s paid N censuses and passed through N−1 half-moved states; `rename` on a directory is this verb. Every move counts **a body reference to an attachment's payload as a reference to its sidecar**: a payload is not a node, but a page that embeds `attachments/photo.jpg` has said where the bytes are, and it keeps showing them after the sidecar or the directory moves. `delete` is the one verb that destroys a document, and it records what it destroyed (see the deletion-log row); `restore` is the graph repair that record is for, not a recovery of bytes. `adopt` links an *existing* file both ways without touching its body — the onboarding complement of `create` (`docs/init-adoption.md`, Phase 1), driving `init --adopt` and the orphan autofix. `duplicate` copies a node as a fresh sibling under the same parent (fresh name, **no** cloned ID or children — a shallow copy, so no child gains a second parent), copying a separated node's body file too. `rename`/`separate`/`combine` each move a document's *own* id from one path to another rather than minting fresh, so each checks `Workspace::move_conflict` — the destination-path half of `registration_conflict`, since the id-direction half would misread the document's own current path as a foreign claim — and refuses before touching the registry rather than silently take the id off whatever the destination already names (`IndexStore::set_path` evicts the displaced id's forward entry as a backstop either way, mirroring `register`'s fix in 11abd38). **Non-goals vs diaryx** (`convert_to_index`/`convert_to_leaf`, `attach_and_move_entry_to_parent`): these reify diaryx's *directory-shaped* containment — a node earns children by becoming a folder, attaching moves the file into the parent's directory. prov's containment is link-shaped (§3, §8): a node gains contents in place and `adopt` links without moving, so there is nothing to convert between and no move-on-attach. The external id-sync hooks (`sync_*_metadata`) are folded into the per-op index maintenance behind the `IndexStore` seam (§9) |
| Validation | `validate` | ✅ findings: broken link (a body `![alt](target)` image included — a picture is not a node, but the page has said where it is, and the census resolves it **by path only**, never through the title index, since no title can make a missing file appear), case mismatch, duplicate containment, missing inverse, unreadable, malformed/dangling id, ambiguous alias, **id mismatch** + **unregistered id** (the frontmatter-storage reconcile pair), **orphan** (a content document on disk nothing reachable links to — the onboarding signal, `docs/init-adoption.md`), **missing containment** (the mirror of *missing inverse*, and the finding that lets a reachability-bounded check see a disconnected island: an unreached document whose `part_of` names a parent in the tree that does not list it — see the Scan row), **fixity mismatch** (see the fixity row), **legacy body hash** (a count of reached documents recording a `content_hash` of their own body, the coverage the retired `fixity: all` tier wrote — one finding for the whole population, since reporting each would mean one per document in exactly the workspaces that have it), **config issue** (a key in either config surface — the root's `prov:` block or the linked config document — that `WorkspaceConfig::apply` silently ignores: a misspelled key that resembles a real axis, or a recognized axis with a value prov can't parse; `config::diagnose` is the shared judgment, so `prov config <key> <value>` refuses to *write* the same, and near-miss detection leaves user-owned fields alone per §2), **config spec-ahead** (a surface declares a `spec` newer than this build understands, so newer settings may be ignored — `config::spec_ahead`, shared with the CLI's proactive warning), **spanning-not-single-parent** (a declared spanning relation whose inverse is `cardinality: many`, §3), **malformed store** (a registry/deletions/*flat*-vocabulary pointer resolving to a markdown doc rather than a whole-file store, §5; a reified vocabulary is content and exempt), **unknown term**/**term near-miss** (a closed-field value that is not a known vocabulary term / an open-field value that resembles one — the self-description layer's term-consistency pass). **legacy deletions pointer** (a root reaching its deletion log through `recycle_bin`, the spelling `deletions` replaced — nothing is broken, but the key outlived its meaning and whatever the bin parked under `items/` is parked on the strength of it). **Severity** is the finding's own (`Finding::severity`, `Error` or `Warning`): an error is a claim the workspace makes that is not so, a warning is drift from a claim it still keeps (case mismatch, stale label, term near-miss, confirmation stale, config spec-ahead, the two legacy reports) — decided once in the library so a consumer inherits the line rather than keeping a list of kinds, and what `check --ignore-warnings` narrows the exit code to. Orthogonal to a repair's `Warrant`, which is about the fix and not the finding. A relation `LinkSite` carries the item's index in a list field (`contents[3]`), so two findings about the same repeated target are told apart. Autofix: missing inverse ✅; id mismatch → trust the registry (rewrite frontmatter) ✅; unregistered id → adopt into the registry ✅; fixity mismatch → re-stamp to the current bytes (confirmation-gated) ✅; missing containment → list the child in the parent it already claims (`Derived`: the document said where it belongs, so nothing is being chosen, and one `--fix mechanical` run reattaches a whole island) ✅; orphan + config-issue + config-spec-ahead + body-link + legacy-deletions-pointer + legacy-body-hash findings stay diagnosis-only (renaming the pointer is one edit, but it un-parks whatever the old bin still holds, so the order is the author's to drive: restore or forget first, rename second) |
| Fixity (content checksums) | `prov_graph::fixity`, `attach`, `validate` | ✅ the archival integrity question link-checking cannot answer — *are the bytes still the bytes?* A NIST-vector-tested SHA-256 through `sha2` (WASM-clean, spelled `sha256:<hex>` so an auditor verifies it with `sha256sum` — tool-agnostic like everything else). `check` grows a bit-rot pass over the reachable set, emitting `FixityMismatch`; it honors any recorded hash regardless of the fixity *setting* (which governs what is written), never false-alarming on an unrecorded or unrecognized digest. **What a checksum covers is not configured** — it is read off the document's shape by `Fixity::covers`: a `content_hash` is written exactly where it covers *a file other than the one recording it*, which is an attachment sidecar pinning its payload, a separated node pinning its prose body, or a manifest node pinning its manifest (that last through the manifest verbs, which alone know what rebuilding it costs). A combined document gets none. That line is the `sha256sum` line and it is drawn there deliberately: a hash of a sibling file is one artifact vouching for another and an outsider reproduces it with a standard tool, while a hash of a combined document's body covers `Document::body` — a parsed substring, and not reliably a contiguous one, since a metadata block need not sit at a file's edge and the body is then the prose on both sides of it concatenated. There is no file to hand `sha256sum` and no rule to state short of reimplementing prov's parser, so the guarantee silently changed strength with the carrier. That is what retired the coverage tier: the axis was `fixity: off \| attachments \| all` and is now `fixity: off \| on` (default **on**; `attachments` still read as a synonym, `all` deliberately not — it asked for the coverage that went away, so it is reported rather than quietly narrowed). Because a covered body is a separate editable file, `prov edit` opens `$EDITOR` and restamps on save, and an out-of-band change is a re-stampable finding rather than a hard error (`Workspace::restamp_fixity`). Body hashes a workspace kept from the `all` tier are still *verified* — a hash on record is always checked — but nothing restamps one, so `check` reports the population once as **legacy body hash** and leaves the two defensible answers (unset the field, or keep it and re-stamp on demand) to the author |
| Provenance (`generated`, `confirmed`) | `provenance`, `mutate::confirm`, `validate`, CLI | ✅ the keeper's own ledger — *who wrote this, and who has read it and found it right* — as two frontmatter families (`docs/provenance.md`). `generated: {by, at, how?}` is written once by whatever created the document and only ever *read* — `how` is the act (`drafted`, `transcribed`, `imported`), the key that tells a note the actor composed from one it carried across, carried without terms of prov's own and closed by a `fields: generated.how:` declaration when a workspace wants it closed; `confirmed: [{by, at, of?}]` is an append-only list `prov confirm` adds to and nothing rewrites. **`check` is the attester and `confirmed` is the record**: a finding is a claim about state, computed now and thrown away; a confirmation is a stored claim about meaning — so prov never writes one from a passing check, and no finding consults the list before firing. A confirmation is **bound to the `updated` stamp** rather than to a digest, because since the fixity coverage rule a combined document has no digest to bind to: an entry is stale once the stamp is newer than its `at` (a string comparison, both fixed-width RFC 3339), and, on the three digest-bearing shapes, once the `content_hash` it named as `of` has moved. `confirm` never stamps `updated` (the stamp is what it is measured against) and refuses a drifted document (a confirmation cannot vouch its way past a fixity mismatch — `stamp` first). A **bare actor is a person** and only a non-human is prefixed (`agent:`, `process:`): the burden goes to the party that can bear it mechanically, at the accepted cost that a tool forgetting its prefix is counted as a person. The tier (unconfirmed · machine-confirmed · human-confirmed) is **derived from the entries that still stand and never stored**. Who is at the keyboard is a device fact — `--by` > `PROV_ACTOR` > an `actor` file beside the peer map — never config, and no actor is a refusal, not an anonymous entry. `ConfirmationStale` fires only while *nothing* stands (a re-confirmed document keeps its older entries as history without being reported), and is diagnosis-only. Deliberately **unsigned**: this is the first-person record, as trustworthy as `author`; the signed second-person vouch — a reviewer's own record the keeper cannot edit — is another tool's subject, and the verb is named `confirm` so the two are not confused |
...
| Not re-reading across verbs | `workspace::inbound` | ✅ **Inbound index**: the census's inverse (target → the documents whose links resolve to it, by path or by id), kept in memory on the `Workspace` between verbs so `retitle` and `rename` find their inbound set by lookup — O(*k*) reads for *k* inbound links, where each call was O(*n*) over the reachable tree. Built from the census the first time a verb asks (one walk, §8's definition of reachable, no second traversal to drift); every ask **stats** every document it knows before trusting itself, and any change drops it; every `apply_set` re-derives the edges of the documents it rewrote from the staged bytes, and drops the index when the set could have changed reachability or id resolution (§5 says where it lives and what invalidates it). Pinned by `a_second_retitle_reads_only_what_it_writes` over a counting backend. A backend priced per call is told when the census starts and ends through the **bulk-read hook** (`prov-graph/src/bulk.rs`, `BulkReads`; §5), announced by the walk and closed before any set lands — pinned by `a_census_is_announced_to_the_backend_and_closed_before_the_set_lands` |
...
| Crash-atomic change sets + journal | [`fs-transaction`](https://crates.io/crates/fs-transaction) | ✅ **error atomicity** (in-memory unwind on any failed write) **and crash atomicity**. Per file: `write_atomic` means no document is caught half-written even by a power cut. For the whole set: a checksummed write-ahead journal — a single transient root dotfile (`.prov-journal` — prov *configures* that name through `journal::workspace_journal`, since the crate's own default is `.fstx-journal`, and an apply and a recovery that disagree about it fail silently), written and flushed *before* any document (the commit point), removed on success — that `recover` (run by `check`) rolls forward idempotently after a crash. A tree something **syncs** is given a **journal home** instead (`WorkspaceBuilder::journal_home`, `Workspace::set_journal_home`): the same `.prov-journal`, kept in a directory the caller owns and nothing syncs, so the tree never holds a journal — not after a crash, and not for the length of an apply. The journal is one machine's crash state; a sync service cannot tell it from content, carries it to machines that never crashed, and there a recovery would replay another machine's intent and every apply would refuse a journal no local change left. A homed journal is recovered by `Workspace::recover_journal` (or `journal::recover_kept_in`), not by `check`, which knows no home; a relative home is refused at the first write, before anything lands; and one home per root is the caller's to keep, since the journal does not record which tree it was for. Whether to finish a legacy in-tree journal as well is the caller's decision. An interrupted set therefore always resolves consistent: fully-before on a caught error, fully-after on a crash. A set of **one** op skips the journal entirely, and the ordinary save is one op: a lone op on a backend claiming `atomic_replace` is already indivisible, so there is no multi-file window left for a journal to close, and journaling it would write, flush and delete a second file to restate a promise the op already carries. What that trades away is liveness, not safety — a crash means the op did not happen rather than being rolled forward to having happened — and for a set of one those are the only two states there are. The stale-journal refusal still applies, since a save must not slip past an earlier change recovery has yet to finish. The registry write still rides the same set (§5). One op journals a **payload by reference**: `FileOp::CopyFrom { path, source }` records the source path where `Write` would embed the bytes, so a set that puts a whole captured workspace back costs O(files) of journal instead of duplicating the entire tree into `.prov-journal` at the commit point. Replay stays deterministic only because the source is *required* to be immutable — a content-addressed history blob is that by construction, so replay finds exactly the intended bytes or fails loudly; pointed at a mutable document it would let recovery invent a state the change never intended, so the obligation sits on the caller. Remaining: an IndexedDB backend delegating to its native transaction instead of the journal. **This layer is no longer prov's**: none of it knows what a document is, so it was lifted out to `fs-transaction` — a zero-dependency crate that prov depends on rather than contains. Reaching it used to mean compiling two Zig-backed parsers it had no use for; the crates that know what a document is now sit *above* it |
| Deletion log (a delete that says what it destroyed) | `mutate`, `relation`, `config` | ✅ a first-class, **reachable** member (a `deletions` pointer relation off the root, discovered by `Workspace::deletions_pointer` — the registry's anti-`.obsidian/` move, §5/§6), not an app-private folder. prov does **not** keep the bytes of a deleted document: recovering those is the job of whatever tool the workspace is recorded under, the same division `prov ignore` draws. What no such tool has is the other half — a `prov rm` touches three documents (the file, the parent's spanning entry, the registry the id is retired from), so a file restored by `git checkout` alone comes back with a `part_of` naming a parent that does not list it and an id nothing registers. `delete` therefore writes a record of what it would take to repair that (path, title, id, parent, body) into the log, in the *same* journaled ChangeSet as the removal, so a delete is never recorded without happening or the reverse. `restore` reads one record and does that repair around bytes the caller has already put back — refusing, and saying what to do, when nothing is at the path. Because `id_storage` defaults to `both`, the id travels in the document's own frontmatter, so a sync can hand it to a second document while the record sat in the log — `restore` therefore checks `registration_conflict` in **both** directions (the id resolving elsewhere, and the path already carrying another id), *and* checks the returned file's own `id:` against the record's, since what lands at the path came from outside prov and may not be the document the record is about. `clear_deletions` forgets the records, always explicit. Config axis `record_deletions` (on by default; off is for a workspace that wants a deletion to leave no trace). **This replaced a recycle bin** that moved bytes under an unreached `recyclebin/items/` and moved them back — historica covers keeping the bytes, and prov could only fall behind it, but nothing else knows the graph. An unmigrated workspace keeps working: the legacy `recycle_bin` pointer and `recycle_bin` axis spelling are still read, `items/` stays parked out of every walk while that pointer stands, `restore` still moves parked bytes home when a record names them, `clear_deletions` still destroys them, and `check` reports the old spelling as a rename to make (`LegacyDeletionsPointer`). Retired with the bin: `recycle`/`recycle_with`/`empty_bin`, the `RecycledBytesMissing` finding and its pass, and `rm --purge`. An operation run *because* something is already broken reports its effect on integrity as a `CheckDiff` — a `check` before against one after, bucketed **fixed / introduced / pre-existing** — since a bare post-operation list cannot tell the damage it repaired from the damage it caused from the damage it inherited, and only `introduced` is a reason to stop. `check --fix` reports and exits on it |
...
| Single-document edits | `edit` | ✅ format-preserving `set`/`unset` over text — landed through `Workspace::save_document` when the file is in a workspace, the same two stamps as `edit` in one journaled write, and as a bare rewrite outside one, so a loose file keeps working; `prov edit` opens `$EDITOR` and, on a real content change, calls `Workspace::record_content_update` — one crash-safe write that restamps the fixity checksum (for a document that gets one — see the fixity row) and stamps the configured `updated` field (empty = off) with an RFC 3339 UTC instant the CLI supplies (the library stays clockless, DESIGN §2; the CLI's `now_rfc3339` is the workspace's one clock, microsecond-precise and fixed-width so a value is orderable as well as readable). Gated on actual change (byte-compared across the editor), so an open-and-quit stamps nothing |
...
| Declarative views | `prov-views`/`config`/CLI | ✅ the spine is one way through the workspace (single-parent, every document in exactly one place); a **view** is a second, and the same document may appear under several groups — precisely what the spine cannot do. Declared as a top-level `views:` axis (`group`/`by`/`under`/`nest`/`label`/`icon`), so every prov tool reads the same lenses rather than each app namespacing its own block (the promotion out of diaryx's `diaryx.views`, whose real cost was that `diagnose` passes over an app block in silence — unlinted, and legible only to the app that agreed with itself where to look). **There is deliberately no `date` grouping**: `group:` names a field or an ordered chain (first non-empty wins) and `by:` is a *cut over a date value*, so a date view is those two aimed at date fields and the chain (`date_of_document` → `created` → `updated`) becomes a declaration the workspace makes rather than three field names the tool blesses — which is what lets a workspace filing by `taken_on` say so, and what stopped three programs each carrying their own copy of the chain. The cut *validates* rather than slicing (`banana` at year grain is not the group `bana`, `20264` is not 2026); a value that doesn't parse falls to the **ungrouped** bucket, reported rather than dropped, because a view whose entries have all quietly stopped grouping is indistinguishable from an empty archive and the difference is the whole diagnosis. `under:` is a **traversal** of the spanning relation, not a path filter — it survives a rename, a move and a retitle, where `path starts-with "Daily/"` does not and a title match finds `Trips/2026` just as happily; an anchor that names nothing is an error, not an empty result (and resolving is not arriving — a path anchor always resolves, so the walk's own verdict on the anchor node is what settles it); the anchor is any link the workspace resolves — a path, an `id:`, or a title (`[[Tasks]]`), the last through a scoped title index built only for a nominal anchor (`Workspace::select_view` passes one with the parked directories excluded; `prov_views::select` alone scans without them), so a stencil can declare a view before its index exists at any path. `nest:` stays independent of `by:` (MoReq2010 §1.4.5's classification-vs-aggregation split: grouping is a reading decision, filing a writing one, and a picker that reads like a display setting must not change where tomorrow's entry lands). prov **reads views and never acts on one** — a view has no invariant, so no `check` finding can come from a wrong one, which is why this is a crate beside prov rather than a feature inside it; `prov-views` depends only on `prov-graph`, so a view engine is structurally unable to write to the workspace it reads. A **grain** (`by:`/`nest:`) is likewise not a calendar but any *coarsening* — `year`/`month`/`day` cut a date (validating, so `banana` at year grain is not the group `bana` and `20264` is not 2026), `initial`/`{initial: n}` cut the first letters for an A–Z index (by **character**, so `Ålesund` is one letter and not a panicking byte slice, and upper-cased, since an index filing `ada` apart from `Ada` is not an index). New grains are added by the same rule as `where:` predicates — a concrete lens that cannot otherwise be said — which is why a numeric `bucket` is absent: unasked for, and its keys sort lexically as `0, 10, 100, 20`, needing grain-aware group ordering that is really the deferred `sort:` axis. **What a date grain reads is EDTF** (`prov-views/src/date.rs`, over the `edtf-core` crate): an archive is mostly approximate dates — "about 1913", "May 1943" with the day torn off, "between 1918 and 1922" from two censuses — and a grain that only read `YYYY-MM-DD` filed every one of them as nothing, which is not what any of them says. ISO 8601-2 is the standard spelling and a strict superset of the date already read: `1913~` cuts to `1913` beside the `1913` that was sure (the qualifier is dropped, a group key names a shelf and not a value), `192X` is a shelf of its own sorting between `1929` and `1930` (an unspecified digit is kept in the year and refused below it — "some month of 1943" is not a month), an interval `1918/1922` is under every year both ends reach, like a letter about two people, with an open or unknown end contributing nothing (`../1920` is under `1920` alone — under-claiming, never wrong), and `XXXX`, EDTF's own spelling of *undated*, cuts to nothing at every grain: it is the ungrouped bucket said on purpose, and a second shelf named `XXXX` would give the archive two undated labels for one fact. An instant still cuts as the date it starts with. This is where diaryx's `date_of_document: unknown` convention goes — the marker becomes `XXXX`, which the type accepts and a view files as undated, and `unknown` itself becomes a `MalformedDate` whose repair is `XXXX`. **`by` and `nest` generalize unequally, and that asymmetry is prov's invariant showing through**: `by` is a *reading* operation with nothing to keep, so any coarsening will do, while `nest` *writes* into the single-parent spanning relation and so demands two more things — that the grain **chains** (each coarser key determined by the finer one: `2026-07-24` → `2026-07` → `2026`, `Ada` → `Ad` → `A`, which an arbitrary sequence of coarsenings does not), and that the grouping value is **single-valued** (a document listing two people has two homes and nothing may choose between them). Both halves ship: `ViewSpec::nest_route` returns the index *titles* to file under — exactly what `route.rs` addressing takes, so a frontend hands them to `plan_route` and never assembles a path — or `None` when the document cannot be filed — or, for a view declaring **`nest: ref`**, the *link* the record's own grouping field carries, to file under whatever it resolves to: the record names its shelf and the shelf's place in the spine is the rest of the chain, so the chain condition is met by construction and nothing here knows the shelf is a day (proposal `nest-by-ref`; the field must be `type: ref`, and `NestRefNotDeclared` says so when it is not); a grouping key is a `FieldPath`, so `written.on` and `confirmed[].by` group as `people` does; and `nest:` over a field declared `type: seq` is a `ConfigIssue::NestNotSingleValued`, diagnosis-only because its two repairs (drop the `nest`, or stop declaring the field a `seq`) say different things about the workspace and the second changes how every *other* consumer reads that field. It fires only when `fields` and `views` share a config surface, `diagnose` being per-surface. **`where:`** narrows what scope reached — `has`/`equals` plus `not`/`any-of`/`all-of`, a multi-key mapping being an implicit and — which is enough for the publishing-audience filtering still entangled in `diaryx_render` and deliberately not an expression language (formulas are the point of no return: a closed set of named predicates grows one member at a time with a reason each, a grammar cannot be taken back). It is a *separate key* from `under:` because the two fail differently — an anchor naming nothing is a broken view, a condition matching nothing is an ordinary empty answer. Three semantics resolve toward saying so rather than guessing, each having two opposite silent readings: an empty or unreadable `where:` is a finding rather than a filter (guessing publishes a workspace or hides it), `any-of: []` matches nothing while `all-of: []` matches everything, and `has` means present *and non-empty*. Running a view is two steps kept apart: **select** (I/O — scope then conditions, returning a flat deduplicated set) and **group** (a pure function, no filesystem, so every grouping question is testable without a fixture vault); a `RowSet` borrows its `Selection`, so one selection groups several ways at once and the *document* count stays distinct from the *row* count — one document under two of a multi-valued field's groups is not two documents. `prov views` lists, `prov views <name>` executes, and `--json` gives either as records — the listing one object per declared view, an executed view its groups, its ungrouped bucket and both counts, with every row carrying the document's **whole metadata block**, since a consumer replacing a per-file `prov meta` loop with a view was reading that metadata and handing it back a path list would put the loop straight back. `prov docs` (`prov_views::documents`) is the **census** every view narrows — every reached document, root included, path order, the same row shape plus an `id` column filled from the document or the registry — offered without a `ViewSpec` because a consumer building its own table over the workspace has no lens to declare; it shares the walk with `select` so the two cannot disagree about which files are documents. `--body` adds each document's prose as a column, read through `Graph::body` so a separated node's row carries its sibling's text and a sidecar's is `null` — the census as a corpus, for a search table or a context dump, and still §2 tier 3: the body is *carried*, never read, exactly as `prov body` carries it one document at a time. **`prov search`** (`prov_views::search`) is the view over that corpus: plain words matched as substrings, case and diacritics folded, over the title, the metadata values and the body of every reached document, ranked by how much of the query the title holds, then the fields, then path order, with the passage cut from the body when a word is there because the title is on the row already. It is two halves like `select`/`group` — a pure scan over folded text that answers *which*, unit-testable without a filesystem, and a body read back per hit for the passage, since holding the raw text beside the folded would be a second copy of the archive for the dozen documents a query shows. A metadata key is left unread when it carries structure — every relation the workspace's own `RelationSet` declares (a link's label is another document's title, so `part_of` would hand every child its parent's name), `id`, `content`, `manifest`, `root`, and whatever pair a consumer adds — and the relation set rather than a list says which, so a workspace spelling containment `whole`/`part` is read right unasked. The index is **derived and disposable**: built from the files, held in memory by whoever asked, thrown away, never written into the workspace as anything a file depends on — which is what keeps this tier 3 rather than a reasoning about the body, and why a workspace opened on a second device searches after a rebuild, not after a sync. ⏳ no `sort:` axis (ordering is ascending-lexical; a date reader wants newest-first) and no formulas — both deferred as the places the format grows teeth |
| Gated exports (what may leave) | `prov-exports`/`config`/CLI | ✅ everything else prov reads is open by default — a view with no `under:` covers the whole workspace, the spanning walk reaches everything. An **export** is the boundary where that flips: a named, **closed-by-default** set of documents that may *leave* the workspace, declared as a top-level `exports:` axis (`label`/`gate`/`view`) beside `views:`. A document is in an export only if the document *itself* declares the export's gate value (`gate: { field: audience, value: family }`), and a document that declares nothing leaves in nothing — the declaration lives in the document, so it travels with the file and still means what it meant, where view membership is a property of the workspace and cannot be. **A gate is not a kind of filter a view could grow; it is a position**: the domain every view runs over once the corpus leaves. The two also invert prov-views' whole rationale — a view has no invariant (a wrong view is a wrong grouping you fix in the picker), while an export is *all* invariant (a wrong export is a file in hands it was never meant for), which is why the valve lives in its own crate rather than as a `where:` idiom: **an export's document set is a subset of its gate's admitted set, whatever the named view says.** `plan` enforces that structurally — entries are seeded from what the gate admits and the view's selection is only ever `retain`ed against, never iterated to add — and everything else fails closed to match: an entry `parse` cannot make a gate of is not an export (it exports nothing; the lint keeps the reason, under the same parser/linter parity test prov-views uses — the silence matters more here, since a gate typo that holds documents back is one nobody notices), and a named view that is unknown or broken is an *error*, never a fall-back to the gate's whole set, because a bound that was written down and cannot be applied must not widen. **A gate is exactly one field and one value, matched exactly after trimming** — not an any-of list, not a condition: the property that makes an export auditable is that "does this document leave?" is answerable by reading one field on that one document, and every richer shape trades it away (the same one-member-at-a-time discipline as `where:` predicates and grains). The one other field an export reads is its optional **`hold`** (`hold: draft`): a document the gate admits that declares the literal `true` under it is *held* — not in the export, reported as `held` beside `withheld` and `outside_view` — because who a document is for and whether it is ready are different facts with different lifetimes, and taking the audience away to say "not yet" loses the first to say the second; the hold narrows and never widens (a withheld document's hold field is never read), fails closed like the gate (a `hold` naming no field is an unreadable export), and keeps the audit property at two named fields on the one document. Exact matching is the fail-closed direction — `audience: Family` under a `value: family` gate is *withheld*, because casing drift is the closed-vocabulary lint's finding to report (`fields.audience.vocabulary` + `UnknownTerm`/`TermNearMiss`), not the gate's to forgive; the gate field wants a closed vocabulary for exactly this reason. Deliberately **no `index:` front-page key** (a rendering concern — an OCFL or copy-out consumer has no front page; the publish layer that wants one owns a render to declare it in, which is diaryx's site layer, whose shape this generalizes — MoReq2010 keeping access control as its own service beside classification and aggregation). `compose` is pure and `plan` does the I/O (`select`/`group`'s split, reused because an invariant should live in code with nothing to mock); the crate depends on `prov-graph` + `prov-views` only, so the layer that decides what may leave is structurally unable to alter what it judges. `diagnose` translates the crate's judgment per surface, plus one cross-key check — an export arranged by a view its surface does not declare, spelled as an `InvalidValue` whose accepted spellings are the declared view names, so the typo is caught at author time rather than as a refusal at publish time. `prov exports` lists, `prov exports <name>` previews all three sides of the boundary (what leaves; the gate-admitted set the view scoped out, listed in full, since "I tagged it and it isn't there" is unexplainable from the file alone; a count of what the gate held back) and moves nothing. ⏳ every consumer of a plan — publish, copy-out, partial sync, an OCFL export — is deliberately downstream; an any-of gate and a gate-value-is-a-known-term check in `validate` (the empty-export typo, catchable once vocabulary stores are loaded) are the named next candidates |
| Vocabulary self-description (`prov/1` spec) | `config`/`relation`/`workspace` | ✅ the relation vocabulary is **declared in frontmatter**, not just a preset: `spanning` + `relations.<name>.{cardinality,inverse,means}` in the root's `prov:` block, parsed by `WorkspaceConfig::apply` and built by `WorkspaceConfig::relation_set`, which **overlays** the declarations on the diaryx preset rather than replacing it: a declared name is added or redefined per field (unsaid halves inherited from the preset's definition, so a `means:`-only gloss keeps the inverse), `<name>: off` retracts one (the key becomes an ordinary carried field), and a workspace that declares nothing gets the preset unchanged, so a minimal vault spells out nothing. Wholesale replacement is declare-plus-`off` — expressible, but written down, since the replace-everything reading meant a block declaring one new pair silently took the spanning tree with it. The five **pointer** relations are exempt from `off` in one direction only: retracting one drops it from the vocabulary, and prov still reads the root's key by that name to find its machinery (§6). `means` is a tier-3 human gloss, carried never read (§2); an *undeclared* preset relation falls back to prov's own gloss (`RelationSet::diaryx_means`), which is true by construction, while a declared one with no `means` stays blank. The single-parent invariant is linted (`SpanningNotSingleParent`), as is a spanning relation the same surface turns off. The five-rule bootstrap kernel + placement rules live in `docs/spec.md` |
| Controlled-vocabulary fields (tags, audiences) | `config`/`vocabulary`/`validate` | ✅ a `fields: { <field>: { values: open\|closed, vocabulary: <pointer>, reify, default, under } }` block — or a list of such declarations, each `under:` an index — turns a carried field into a *resolvable, checked* reference (§2: consistency = resolvability). A declaration is a property of a **region** of the tree, not of the workspace: `Workspace::field_scopes` resolves each `under` as a view's anchor is resolved and collects its subtree, and `FieldScopes::spec_for` answers which declaration governs a document (the deepest containing scope, else the unscoped one; the index is not in its own scope). Every reader asks it first — `check`'s term pass, `new`'s starting values, the widen-vocabulary repair, `about.md`'s tables — so `status` is four closed terms under `Tasks` and five under `Proposals` with nothing between. A **flat** vocabulary is a whole-file store (a self-describing node, `Vocabulary::from_meta`) with a `terms:` map; a **reified** one (`reify: true`) is an ordinary content index node whose spanning children are term documents, loaded by `Workspace::load_reified_vocabulary` — no `vocabulary:` marker and no whole-file demand, since the store is content and that is the point (spec §3, §4). Either way prov reasons about term keys, each `id`, and `retired`, carrying the rest (a diaryx audience's gate/theme payload) — for a reified term the `id` is the *node's own*, and `Workspace::reified_term_path` is how a consumer reaches the node to read the rest off it. `check` grows `vocabulary_findings` over the reachable set: `UnknownTerm` (closed) / `TermNearMiss` (open, reusing the config linter's edit-distance via `textdist`). Diagnosis-only. **`type: date` is the one declared type a value is held to** — `date_findings` walks the same set with a parse in place of a vocabulary, and a value that is neither a calendar date, an instant, nor EDTF is a `MalformedDate`: what a date view *silently* leaves ungrouped, named. The repair is `edtf-normalize`'s reading of the prose — `May 1943` → `1943-05`, `c. 1913` → `1913~`, `unknown` → `XXXX` — one remedy per reading (`5/12/1943` is offered as May and as December, and neither is assumed), every one a judgment, and none where the prose is outside its grammar. `datetime` is not checked: it is what a machine stamps, and a stamp's fractional seconds are not EDTF. A declaration's path is a `FieldPath` (`prov-graph/src/field.rs`) — a key, a key inside a mapping, or a key inside **every item of a list** (`confirmed[].by`) — walked by `values_at`/`strings_at` to every value it reaches, each with the concrete `Address` (`sources[2].resource`) a finding names and a repair edits, so the path grammar is its own address grammar. **`type: ref`** is the one type prov reads: `WorkspaceConfig::reference_fields` puts each such path in `ReadSettings::references`, and `Graph::frontmatter_links` enumerates relation entries and ref-field values together as the one list the census, the inbound index, `mv`'s re-relativizing, `retitle`'s relabel and `convert`'s restyle all walk — a `LinkSite::Field` carries the address whole. One-way by design (no inverse, never spanning, unscoped like a relation's name; `ScopedReference` says so): *a link that stands alone is a relation; a link that sits beside other facts about itself is a path-valued field* (proposal `path-valued-fields`) |
| Whole-file record stores | `document`/`index`/`mutate`/`validate` | ✅ registry, deletion log, and flat vocabularies must be whole-file config documents — `require_whole_file` refuses a markdown carrier at load, `check` reports it (`MalformedStore`). Prose routes to *reified* term nodes instead (§5) |
| Presets | `preset` / CLI | ✅ a **stencil, not a default**: `Preset` is a directory laid out like the workspace root — a `prov.yaml` carrying only the axes it declares and, beside it, the stores those axes link — and applying it writes all of that out, in full, into the workspace's own config and files, after which the reader has nothing to expand (`docs/proposals/presets/proposal-presets-v1.md`). prov ships exactly one, unnamed, in code (`Preset::builtin`: `created` and `updated`), and it is a preset rather than a hard-coded default so that `init --preset <dir>` can *replace* it; there is no registry and no list of names for a second entry to join. `plan_preset` judges every entry and store against the config as *declared* — raw, root block under config document — so a preset is compared with what the author wrote and not a normalized re-spelling: absent or at the default → `Set`, same → `Same`, different → `Differs`; a store absent → `Write`, identical → `Present`, different → `Occupied`. `apply_preset` refuses a plan with any collision and lands a clean one in one change set. `prov presets [DIR] [--write]` prints the plan either way; `init` applies the built-in (or `--preset`) through the same seam, stripping an axis a flag named explicitly. This repository's `presets/tasks/` is the fixture, applied to a scratch workspace and `check`ed. Scoped declarations (the controlled-vocabulary row) are what let the `tasks` preset declare `status` per index. |
