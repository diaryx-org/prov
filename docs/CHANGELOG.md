...
Every notable change to the prov workspace, newest first. Every published crate
in the workspace shares one version number and one release, so this is the
changelog for all of them; the scope on a bullet (`**graph**`, `**cli**`) says
which surface moved. How many crates that is has changed with almost every
release — `cargo publish --workspace --dry-run` is the current answer, and no
prose here tries to be.
...
(Eleven crates at 0.5.0. `prov-history` was retired in the release after 0.7.0,
and `prov-transaction`, `prov-identity` and `prov-fixity` with it.)
...
- `LinkSite::Relation(String)` is now a struct variant
  `LinkSite::Relation { field, index }`, and `prov_graph::relation::Edge` has
  a new `index` field; code that matched or constructed either needs editing.
  A broken-link message for a list item now reads `broken contents[3] link: …`
  where it read `broken contents link: …`.
